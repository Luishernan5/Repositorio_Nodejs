...
const {escaparCaracteres} = require('./escape')
...
function grabarEnArchivo(formulario) {
  const datos = `nombre:${escaparCaracteres(formulario.get('nombre'))}<br>
              comentarios:${escaparCaracteres(formulario.get('comentarios'))}<hr>`
